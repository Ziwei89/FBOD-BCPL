...
def pth_to_onnx(input, checkpoint, onnx_path, input_names=['input'], output_names=['output'], device='cpu'):
    ...
    model.load_state_dict(torch.load(checkpoint)) # Initializing the weights
    model.eval()
    model.to(device)
    
    torch.onnx.export(model, input, onnx_path, verbose=True, input_names=input_names, output_names=output_names) # Specifies the input to the model, and the output path to onnx
