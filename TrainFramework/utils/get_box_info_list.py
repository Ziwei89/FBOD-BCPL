...
def object_score_to_HElevel(object_score): # Convert object score to Hard Easy Level
    if object_score>=0.25: # easy
        return 0
    else: ### hard
        return 1
...
class Box_info(object):
    ...
    def prior_score(self):
        return self.__prior_score
    @prior_score.setter
    def prior_score(self,score):
        self.__prior_score=score
    ...
    @property
    def HElevel(self):
        self.__HElevel = object_score_to_HElevel(self.__prior_score)
        return self.__HElevel
...
class getBoxInfoListForOneImage(nn.Module):
    def __init__(self, image_size, stride=2, gamma=0.25, cuda=True):
    ...
    def forward(self, input, raw_bboxes, bboxes):
       # input is a [CONF, LOC] list with 'bs,c,h,w' format tensor.
        ...
        for box_info in box_info_list:
            ...
            predict_conf = float(predict_conf.cpu().detach().numpy())
            # post_score = (1-γ)*prior_score + γ*predict_conf
            box_info.post_score = (1-self.gamma) * box_info.prior_score + self.gamma * predict_conf
